#!/usr/bin/env bash
# Start all backend services for staging
...
echo -e "${BLUE}Starting ECM Backend Services for Staging${NC}"
echo ""

# Check if Redis is running
if ! redis-cli ping &>/dev/null; then
    echo -e "${YELLOW}Warning: Redis is not running. Please start Redis before continuing.${NC}"
    echo -e "${YELLOW}On staging server, Redis should be managed by systemd or supervisor.${NC}"
    sleep 2
fi
...
"${SCRIPT_DIR}/start-backend-stag.sh" &
BACKEND_PID=$!
...
"${SCRIPT_DIR}/start-celery-stag.sh" &
CELERY_PID=$!
