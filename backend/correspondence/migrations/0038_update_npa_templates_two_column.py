...
def reverse_update(apps, schema_editor):
    ...
    dept = CorrespondenceTemplate.objects.filter(title="NPA Departmental Memorandum").first()
    if dept:
        dept.content_html = DEPARTMENTAL_HTML_PREV
        dept.content_text = DEPARTMENTAL_TEXT_PREV
        dept.save(update_fields=["content_html", "content_text"])

    internal = CorrespondenceTemplate.objects.filter(title="NPA Internal Memorandum").first()
    if internal:
        internal.content_html = INTERNAL_HTML_PREV
        internal.content_text = INTERNAL_TEXT_PREV
        internal.save(update_fields=["content_html", "content_text"])


DEPARTMENTAL_HTML_PREV = """<section style="font-family: Verdana, Geneva, sans-serif; line-height: 1.5; color: #000;">
...
INTERNAL_HTML_PREV = """<section style="font-family: Verdana, Geneva, sans-serif; line-height: 1.5; color: #000;">
...
DEPARTMENTAL_TEXT_PREV = "NIGERIAN PORTS AUTHORITY\n{{division.name}}\n{{department.name}}\nDepartmental Memorandum\n\nDate: {{date.today}}\nTo: {{recipient.name}}\nFrom: {{sender.name}}\nRef.: {{document.reference}}\n\nRE: {{document.title}}\n\nDear Colleague,\n\n[Insert body content here]\n\nYours sincerely,\n\n{{sender.name}}\n{{sender.title}}"

INTERNAL_TEXT_PREV = "NIGERIAN PORTS AUTHORITY\n{{division.name}}\nInternal Memorandum\n\nDate: {{date.today}}\nTo: {{recipient.name}}\nFrom: {{sender.name}}\nRef.: {{document.reference}}\n\nRE: {{document.title}}\n\nDear Sir/Madam,\n\n[Insert body content here]\n\nYours faithfully,\n\n{{sender.name}}\n{{sender.title}}"


class Migration(migrations.Migration):
    dependencies = [
        ("correspondence", "0037_update_npa_templates_dynamic_header"),
    ]
