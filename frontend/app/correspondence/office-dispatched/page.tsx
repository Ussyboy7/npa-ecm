  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await apiFetch<Record<string, unknown>>(
        `/correspondence/items/office-dispatched/?${params.toString()}`
      );
      ...
      exportToCSV(
        ...
        { filename: `office-dispatched-export-${new Date().toISOString().split('T')[0]}.csv` }
      );
  ...
  useEffect(() => {
    ...
    const fetchDispatched = async () => {
      ...
      try {
        ...
        }>(`/correspondence/items/office-dispatched/?${params}`, { signal: controller.signal });
