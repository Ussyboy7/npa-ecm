...
import { Shield, Plus, Pencil, Eye, Trash2, Search } from "lucide-react";
...
export default function DrmPoliciesPage() {
  ...
  const statItems = useMemo(
    () => [
      { key: "total", label: "Total Policies", value: policies.length },
      { key: "active", label: "Active", value: policies.filter((p) => p.is_active).length },
      { key: "inactive", label: "Inactive", value: policies.filter((p) => !p.is_active).length },
    ],
    [policies],
  );
      ...
      actions={
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild>
            <Button size="compact" onClick={() => resetForm()}>
