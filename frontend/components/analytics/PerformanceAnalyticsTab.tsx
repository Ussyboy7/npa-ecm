...
export const PerformanceAnalyticsTab = () => {
    ...
    const load = async () => {
      ...
      try {
        const [response, targets] = await Promise.all([
          fetchPerformanceAnalytics(selectedPeriod),
          fetchSLATargets(),
        ]);
        if (!ignore) {
          setData(response);
  ...
  const avgSLATargetDays = useMemo(() => {
    ...
    // Weighted average: urgent (10%), high (20%), medium (50%), low (20%)
    const avgHours = (slaTargets.urgent * 0.1) + (slaTargets.high * 0.2) + (slaTargets.medium * 0.5) + (slaTargets.low * 0.2);
    return Math.round((avgHours / 24) * 10) / 10; // Convert hours to days, round to 1 decimal
  }, [slaTargets]);
